...
async function estimateCO2(apiKey, contracts) {
    ...
    const networkGasUsed = csvRows.map((row) => {
    ...
    const networkEmissions = (await httpsGet("https://kylemcdonald.github.io/ethereum-emissions/output/daily-ktco2.csv"))
        .split("\n")
        .slice(1)
        .map((row) => {
    ...
    report.byDate.forEach((byDate, date) => {
        ...
        report.byAddress.forEach((byAddress, address) => {
            if (byAddress.byDate.has(date)) {
                const byAddressAndDate = byAddress.byDate.get(date);
                ...
                byAddressAndDate.bySelector.forEach((byAddressAndDateAndSelector, selector) => {
